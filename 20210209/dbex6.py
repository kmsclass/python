...
cur.execute('''create  table items (
        ...
        name varchar(300),  price integer)''')
...
print("items 테이블에서 가격이 3000이상 10000이하인 레코드를 조회") 
#cur.execute("select * from items where price between %s and %s",
#                                             (3000,10000))
cur.execute("select * from items where price >= %s and price <= %s",
                                             (3000,10000))
for row in cur.fetchall() :    
    print(row) 

#items 테이블에서 가격이 30000원인 레코드 제거하기
print("items 테이블에서 가격이 30000원인 레코드 제거하기") 
